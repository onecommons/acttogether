...
function renderClientside(_ctx, opt) {
  var out = '<script>$.templates = {}</script>'; //XXX
  if (_ctx.clientTemplates) {
    var templates = {};
    ...
    _.each(_ctx.clientTemplates, function(value) {
      if (!templates[value.template]) {
        ...
        out += "<script async defer src='/jswig/" + path.normalize(scriptpath) +".js'></script>\n";
        templates[value.template] = true;
      }
    });
    out += '<script src="' + path.join( (opt && opt.jsbase) || "/static/js/", "swig.min.js") + '"></script>\n';
