...
module.exports.dataform = function(){
   ...
   // parse select options array of form [['id' : @1, 'name': 'first choice', 'notes':['foo', 'bar']], ...]
   function parseObjectOptionsArray(options, selection, notesJoin){
       notesJoin = (notesJoin === undefined) ? ' ' : notesJoin;
       var rv = '';
       for(var i = 0; i < options.length; i++){
           bindClass = setFormBindClass(options[i]['id']);
           rv += '<option' + (options[i]['id'] === selection ? ' selected' : '');
           if(bindClass !== '') {
               rv += ' class= "' + bindClass + '" ';
           }
           rv += ' value= "' + options[i]['id'] + '" >';
   ...
   df.select = function(prop, options, htmlAttrs){
       ...
       } else if(options[0].hasOwnProperty('id')){
