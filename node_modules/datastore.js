...
var pJSON = {
  ...
  stringify: function(value, space) {
    /*
      ...
      other '_id' keys are serialized with a leading '@'
      ...
      if (v && typeof v === 'object') {
        ...
        for (var prop in v) { 
          ...
          if (val instanceof ObjectID) {
            ...
            if (typeof val == 'string' && (val.charAt(0) == '@' || val.slice(0,2) == '::')) {
             val = '::' + val;
            }
            if (prop == '_id') { //user defined ids
              val = '@' + val;
            } 
          }
          copy[prop] = val;
  ...
  reviver: function(k,v){
      ...
      if the key is _id, remove the leading @ if present, otherwise:
      string values that look like @*@* or @*@@* are converted to DbRefs with a user-defined string id or ObjectId respectively, unless the key is _id
      string values starting with @ are left as is (should be treated as user-defined id references in the current collection)
      strip the leading :: from strings
      */
      if (typeof v == 'string') { 
        ...
        if (prefix =='@@') {
          ...
          if (prefix.charAt(0) == '@') {
            if (k == '_id')
              return v.slice(1);
            //XXX check for escaping to handle user-defined ids and collections with @ in their name
            var index = v.indexOf('@', 1); // /(?!::)@/
            if (index > -1) { //DBRef
              if (v.charAt(index+1) == '@')
                return new DBRef(v.slice(1, index), ObjectID(v.slice(index+2)));
              else
                return new DBRef(v.slice(1, index), v.slice(index+1));
...
var MongooseDatastore = function(modelMap) {
  this.models = modelMap || mongoose.models;
};
...
MongooseDatastore.prototype = {
  ...
  */
  ...
  ,add: function(mjson, cb) {
    ...
    model.findOne({ _id: mjson._id}, function (err, obj) {
      if (err) {
        cb(err);
        return
      }
      ...
      for (var key in mjson) {
        ...
        var val = mjson[key];
        if (Array.isArray(val)) {
          //XXX error if !(obj[key] instanceof MongooseArray) //XXX unit test
          obj._doc[key].extend(val);
        } else {
          obj._doc[key] = val; //XXX is there a cleaner approach than using _doc? 
          obj.markModified(key); //explicitly mark in case object has "mixed" schema //XXX unit test
        }
      }
      //console.log('found', mjson, obj._doc);
      obj.save(cb); 
    });
  }
